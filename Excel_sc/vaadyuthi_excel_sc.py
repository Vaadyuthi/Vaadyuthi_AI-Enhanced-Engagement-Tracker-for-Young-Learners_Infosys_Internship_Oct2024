...
if not os.path.exists("vaadyuthi_screenshots(4)"):
    os.makedirs("vaadyuthi_screenshots(4)")

# Load the known image of Vaadyuthi
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"Vaadyuthi_screenshots(4)/Vaadyuthi_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for Vaadyuthi with screenshot
                ...
                for face_location in face_locations:
                    ...
                    cv.putText(frame, 'Vaadyuthi', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Vaadyuthi'
            cv.putText(frame, 'Not Vaadyuthi', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('vaadyuthi_attendance_with_screenshots(4).xlsx', index=False)
            print("DataFrame saved to 'vaadyuthi_attendance_with_screenshots(4).xlsx'")
    ...
    if not df.empty:
        df.to_excel('vaadyuthi_attendance_with_screenshots(4).xlsx', index=False)
        print("Final attendance and screenshots saved to 'vaadyuthi_attendance_with_screenshots(4).xlsx'.")
