...
if not os.path.exists("Vaadyuthi_screenshots(5)"):
    os.makedirs("Vaadyuthi_screenshots(5)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"Vaadyuthi_screenshots(5)/Vaadyuthi_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
        ...
        if (datetime.now() - last_saved_time).total_seconds() >= 30:
            df.to_excel('Vaadyuthi_attendance_with_screenshots_dt(5).xlsx', index=False)
    ...
    if not df.empty:
        df.to_excel('Vaadyuthi_attendance_with_screenshots_dt(5).xlsx', index=False)
        print("Attendance and screenshots saved to 'Vaadyuthi_attendance_with_screenshots_dt(5).xlsx'.")
